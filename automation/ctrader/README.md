...
- 계좌의 확정 잔액과 보유 포지션 개수를 자동 기록에 표시한다. 미실현손익을 포함한 평가금액이 아니다.
- 각 포지션의 진입 주문 상세와 과거 캔들을 조회해 진입·분할 청산·최초 손절·익절을 표시한다. 5분봉을 기본으로 기간이 길면 15분/1시간/4시간/일봉으로 넓힌다.
- 차트 요청은 기간만 지정한다. 실제 브로커에서 `count`를 주면 시작 구간보다 앞선 캔들을 반환하므로 함께 지정하지 않는다. 잘린 응답과 누락값은 정상 차트로 취급하지 않는다.
- 한 번의 진입으로 끝난 완전한 거래에서 최초 주문의 손절을 확인한 경우만 가격 기준 비용 전 R을 계산한다. 분할 청산은 수량 가중한다. 최초 주문이 진입 후 수정됐거나 손절 정보가 없으면 R을 추측하지 않는다.
- 손절/익절 주문은 보호 주문의 가격과 실제 체결가를 대조한 추정 표시다. 일반 청산 주문을 사용자 직접 클릭으로 단정하지 않는다.
- 종료된 거래의 차트는 캐시하고 정기 전체 조회 때 다시 확인한다. 거래별 복기는 독립 체결 통계를 더한 값이 아니라 포지션 전체의 결과다.
- 차트는 FP Markets 시세로 재구성한다. 개인 TradingView 지표·그림은 복원하지 않는다. 사용자가 이미지 또는 TradingView 스냅샷/차트 링크를 메모에 첨부할 수 있다. 이미지는 메모당 3장, JPEG 압축 후 45만 문자 이하로 제한하고 기존 일지 백업·동기화에 포함한다.

추가 검증: `node --test automation/ctrader/review.test.mjs automation/attachments.test.mjs`.
