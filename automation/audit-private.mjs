...
import { readVault } from './ctrader/vault.mjs';
try {
  const [note, connection, fpDirectory] = process.argv.slice(2);
  const credentials = credentialsFromNote(await readFile(note, 'utf8'));
  const code = JSON.parse(await readFile(connection, 'utf8')).code;
  const fp = fpDirectory ? await readVault(fpDirectory) : null;
  const privateValues = [credentials.clientId, credentials.clientSecret, code,
    ...['clientId','clientSecret','accessToken','refreshToken','redirectUri','accountId'].map(k=>fp?.[k]).filter(Boolean)];
  ...
  for (const path of files) {
    if (!/\.(js|jsx|mjs|json|html|css|md|ps1|sql|yml|yaml|example|java|xml|gradle|properties)$/.test(path)) continue;
    const text = await readFile(path, 'utf8'); checked++;
    if (privateValues.some(secret => text.includes(secret))) {
