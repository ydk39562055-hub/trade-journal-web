...
오늘의 주문
· 확장 + 편향 + 스윕 + OF전환 — 4개 모두 맞을 때만 진입. 하나라도 빠지면 관망.
· 가격(PRICE)을 읽고, 맞는 시간(TIME)을 기다린다. 관망도 포지션.
· 오늘 못 먹어도 된다. 규칙 지키는 게 이기는 것.
· 한 번에 한 셋업. 추격·복수·물타기 금지.

━━━━━ 0. 데일리 루틴 (매일 읽고 시작) ━━━━━

① 장전 분석 — HTF 컨텍스트
☐ 일봉·4H 오더플로우 방향 (상승 / 하락)
☐ 50% EQ — 프리미엄인가 디스카운트인가
☐ HTF FVG·OB에서 거부 중 vs 유동성으로 달리는 중
☐ True Daily Open(00:00 NY) 표시 → 롱은 시가 아래·숏은 위에서만
☐ 오늘의 DOL 한 곳 확정 — 없으면 거래 안 함

② 레벨 마킹 & 시간 세팅
☐ 유동성 풀 표시 — 아시아 H/L · 전일·전주 H/L · Equal H/L
☐ 차트 NY 시간 세팅 확인
☐ 고임팩트 뉴스 시간 표시 (발표 직전 진입 자제)
☐ 시간 왜곡일 점검 (월요일·월말/분기말 등)

③ 시간 필터 — 대기
☐ 런던 킬존 / 뉴욕 본장 안인가 (밖이면 PASS)
☐ 매크로 윈도우(:50–:10, AM 10–11)에 집중

④ 셋업 포착 — 순서대로 (하나라도 빠지면 PASS)
☐ 1. 스윕 — 키풀(아시아·전일 H/L·Equal) 건드림
☐ 2. SFP — 꼬리로 찌르고 몸통은 라인 안쪽 마감
☐ 3. Displacement — 강한 변위 무빙 (← 아래 둘을 만든다)
☐ 4. MSS/CISD + FVG — 반대 스윙 몸통 돌파 + FVG ≥ 0.5×ATR 생성
☐ 5. SMT — 상관자산 괴리 (있으면 버프 / MSS 후 유효)

⑤ 진입 집행 — 기계적으로
☐ 진입 — FVG 상단 또는 50% CE에 지정가
☐ 손절 — SFP 꼬리 아래 / OB 시가 아래
☐ delivery sentence 한 줄 — 가격이 어디서 어디로 갈 것인가
...
⑥ 청산
· 1차 내부 유동성서 50% 익절 + 손절 본전 이동
· 2차 외부 유동성 — 내러티브 안 깨졌으면 DOL까지 보유
