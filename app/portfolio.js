...
(function () {
  ...
  function quoteAge(q) {
    if (!q) return '';
    if (!q.at) return q.state === 'REGULAR' ? '장중' : '';       // 옛 함수(시각 없음) 호환
    const d = new Date(q.at);
    // 야후가 marketState를 안 줄 때가 있어 시각으로 판단 — 20분 이내 값이면 장중으로 봄
    const live = q.state === 'REGULAR' || (Date.now() - d.getTime()) < 20 * 60 * 1000;
