...
/* ───────────── 더보기 (CSV / JSON) ───────────── */
function MenuModal({ entries, syncId, onImport, onReset, onSync, onBackfillR, onClose }) {
  const fileRef = useRefM();
  ...
  return (
    <Modal open onClose={onClose} title="더보기" maxWidth={440}>
      ...
      <button className="btn-ghost" style={row} onClick={onBackfillR}>과거 선물 R 채우기 (대략)</button>
      <div style={{ fontSize: 12.5, color: 'var(--ink-3)', lineHeight: 1.5 }}>R배수가 비어 있는 과거 선물 거래를 손익금액으로 대략 채웁니다. 1R = 선물 시드 × 등급 리스크%(등급 없으면 0.5% 가정). 이미 적은 R은 건드리지 않아요.</div>
      <div style={{ borderTop: '1px solid var(--border)', margin: '14px 0 10px' }} />
