function BrokerPanel({ code, onConnect, memos, onAddMemo, onRemoveMemo, syncId }) {
  ...
  React.useEffect(() => setLimit(40), [search, side, month]);
  ...
  return <section className="broker-panel">
    ...
    {code && <>
      {(view.error || stale || view.status?.state === 'error') && <p className="broker-notice" role="status">{view.error || (view.status?.state === 'error'
      ...
      <p className="broker-explanation">분할 체결은 주문별로 합쳐 보여줘요. 체결 대금은 매매 금액이며, 실현손익 통계에는 아직 합산하지 않아요.</p>
      <div className="broker-list">{selected.slice(0, limit).map(row => <BrokerTrade key={row.id} row={row}
        memos={memos.filter(m => m.brokerTradeId === row.id)} onAddMemo={onAddMemo} onRemoveMemo={onRemoveMemo} />)}</div>
...
function BrokerTrade({ row, memos, onAddMemo, onRemoveMemo }) {
  ...
  return <article className="broker-trade">
    ...
    <div className="broker-trade-date">{row.tradedAtKorea || '날짜 미확정'} · {time} · 토스증권</div>
