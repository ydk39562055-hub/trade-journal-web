...
alter table public.free_journal add column if not exists setups       jsonb default '[]'::jsonb;     -- ICT 셋업 태그(IFVG/OB/SFP 등)
alter table public.free_journal add column if not exists timeframe    text;                          -- 진입 분봉(1분/5분/15분 등)
